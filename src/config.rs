...
impl Config {
	pub fn new() -> Config {
		...
		let display_size = serde_json::from_slice::<(u32, u32)>(settings["WINDOW_SIZE"].to_string().as_bytes()).unwrap();
	...
	pub fn save(&self) {
		...
		let req_str: String = format!("{{ \"requests\": [ {} ] }}", self.requests.iter().flat_map(|s| s.chars()).collect::<String>());
